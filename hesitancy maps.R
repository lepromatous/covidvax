...
#### begin function
yo <- function(datez=Sys.Date()-1, xvar = "estimated_hesitant", yvar = "series_complete_12pluspop", complete.sub = 90){
  sf::sf_use_s2(FALSE)
  #=======================================================================
# hesitancy
...
## make url for socrata
urlz <- paste0("https://data.cdc.gov/resource/8xkx-amqh.json?date=", dt, "&$select=fips, completeness_pct, series_complete_pop_pct, series_complete_12pluspop, series_complete_65pluspop, recip_county,recip_state")
...
covid %>%
  clean_names() %>%
  mutate(across(all_of(names(covid)[2:4]), as.numeric),
...
df %>%
  ...
  mutate(mean_state_hesitancy = round(mean(estimated_hesitant, na.rm=T),1),
         covid_uptake_mean_state = round(mean(series_complete_12pluspop, na.rm=T),1)) %>%
  ungroup() -> df
...
varnames <- c("Percent Fully Vaccinated" = "series_complete_pop_pct", 
              "Percent 12+ Fully Vaccinated" =  "series_complete_12pluspop",
