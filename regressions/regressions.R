...
sf::sf_use_s2(FALSE)
  ...
  covid %>%
    clean_names() %>%
    mutate(across(all_of(names(covid)[c(6:34, 36:40, 42:46)]), as.numeric),
  ...
  ##### clean
  rm(list=setdiff(ls(), c("df", "covid_cases_deaths")))
  gc()
  ...
  # case/death with all
  df.sf <- merge(df, covid_cases_deaths, by=c("fips", "date"),all.x=T)
  ...
  df.subs <- subset(df.sf, df.sf$completeness_pct>=0.9)
 ...
 write.fst(df.sf, "~/Desktop/df.fst")
